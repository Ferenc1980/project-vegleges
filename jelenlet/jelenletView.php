...
if(isset($_POST['megjelenit']) && $_POST['honap']!='0'){
    ...
    foreach($lepedo as $key=>$value){
        ...
        $total=$dolgIdo+$value->osszSzabadsag()*8+$value->osszbeteg()*8;
        $idovaltozo=ido($dolgIdo*60);
        $tartalom.="<td>{$idovaltozo}</td><td>{$value->osszSzabadsag()}</td><td>{$value->osszbeteg()}</td><td>{$total}</td></tr>";
    }
